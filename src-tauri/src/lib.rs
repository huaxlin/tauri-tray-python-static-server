...
// 封装一个启动 Python 服务的函数
fn start_python_server(app_handle: &tauri::AppHandle, port: &str, path: &str) -> Option<CommandChild> {
    println!("正在启动 Python 服务，端口: {}, 目录: {}", port, path);
    
    // 使用我们在权限里配置的 "python3" 规则
    let shell = app_handle.shell();
    let cmd = shell.command("python3").args([
        "-m",
        "http.server",
        port,
        "-d",
    ...
    match cmd.spawn() {
        Ok((_rx, child)) => {
            println!("🟢 Python 服务启动成功！");
            Some(child)
        }
        Err(e) => {
            eprintln!("🔴 无法启动 Python 服务: {}", e);
            None
        }
    }
}
...
pub fn run() {
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            let download_path_buf = app.path().download_dir()
                .expect("无法获取下载目录");
            let default_path = download_path_buf.to_str()
                .expect("路径转换为字符串失败");

            // 4. 启动初始化服务并存入状态机
            let child_proc = start_python_server(&app_handle, default_port, default_path);
                ...
                .on_menu_event(move |app_handle, event| {
                    match event.id.as_ref() {
                        ...
                        "restart" => {
                            ...
                            // 重新拉起新进程
                            let new_child = start_python_server(app_handle, "8080", "/Users/linhuaxing/Downloads");
